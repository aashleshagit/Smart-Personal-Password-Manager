...
class LoginWindow:
    ...
    def search_password_window(self):
        ...
        try:
            results = search_password(keyword.strip())

            # Some versions of database.py return None when
            # nothing is found. Convert that safely to an empty list.
            if results is None:
                results = []

            # If the database function returns one record as a tuple,
            # convert it into a list containing that record.
    ...
    def show_records(self, records, title_text):
        ...
        # Always use a list so an empty search result
        # does not crash the Treeview window.
        if records is None:
            records = []
        elif isinstance(records, tuple):
            # A single database record is a tuple of fields.
            if len(records) >= 6 and not isinstance(records[0], (tuple, list)):
                records = [records]
            else:
        ...
        for row in records:
            ...
            # Make sure this is a complete database record.
